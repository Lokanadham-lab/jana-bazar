...
function home(){
 ...
 $("#app").innerHTML=`<div class="container">
  <section class="hero">
    <div><img class="hero-logo" src="jana-bazar-dark.png" alt="JANA BAZAR"><h1>India's People's Marketplace</h1>
...
function product(id){const p=db.products.find(x=>x.id===id);if(!p)return;modal(`<button class="close" onclick="JB.close()">×</button><img class="product-img" src="jana-bazar-logo.png" alt=""><h2>${esc(p.name)}</h2><p class="muted">${esc(p.seller)} • ${esc(p.sellerType)}</p><p><b class="price">${money(p.price)}</b> <span class="mrp">${money(p.mrp)}</span></p><p>Stock: ${p.stock}</p><h3>Specifications</h3><p>Category: ${esc(p.category)}</p><p>Seller type: ${esc(p.sellerType)}</p><p>Delivery: location-based</p><p>Returns: seller/category policy</p><button class="btn" onclick="JB.add('${p.id}');JB.close()">Add to Cart</button>`)}
function cart(){
...
function admin(){
 modal(`<button class="close" onclick="JB.close()">×</button><img src="jana-bazar-admin.png" style="width:230px;max-width:100%;height:auto" alt="JANA BAZAR Admin"><h2>🔐 Admin Portal</h2>
...
function adminDash(){
 ...
 <aside class="card sidebar"><img src="jana-bazar-admin.png" style="width:100%;max-height:95px;object-fit:contain" alt=""><h3>Admin Control Center</h3>
...
function advertising(){ $("#app").innerHTML=`<div class="container"><div class="card"><img src="jana-bazar-shop.png" style="width:220px;max-width:100%" alt=""><h2>Advertising Center</h2><p class="muted">Campaign submission interface. Billing and ad delivery require backend services.</p><button class="btn" onclick="toast('Campaign queued in demo mode')">Create Campaign</button></div></div>`}
function local(){products(db.products.filter(p=>p.sellerType!=="Brand"),"Local Commerce")}
function services(){$("#app").innerHTML=`<div class="container"><img src="jana-bazar-services.png" style="width:260px;max-width:100%" alt=""><h1>Services</h1><div class="grid">${["Electrician","Plumber","AC Service","Computer Repair","Home Cleaning","Painter","Vehicle Assistance","CCTV Installation"].map(x=>`<div class="card"><h3>${x}</h3><p class="muted">Verified-provider workflow.</p><button class="btn small" onclick="toast('Booking flow opened')">Book Service</button></div>`).join("")}</div></div>`}
function b2b(){$("#app").innerHTML=`<div class="container"><img src="jana-bazar-b2b.png" style="width:260px;max-width:100%" alt=""><h1>B2B Marketplace</h1><p class="muted">Manufacturer → Distributor → Dealer → Wholesaler → Retailer.</p><div class="grid">${db.products.filter(p=>p.category==="B2B / Wholesale"||["National Distributor","Regional Distributor","Wholesaler"].includes(p.sellerType)).map(productCard).join("")}</div></div>`}
function utilities(){$("#app").innerHTML=`<div class="container"><img src="jana-bazar-utilities.png" style="width:260px;max-width:100%" alt=""><h1>Utilities</h1><div class="grid"><div class="card"><h3>📱 Recharge & Bills</h3><p>Provider integration required.</p></div><div class="card"><h3>🚘 FASTag</h3><p>Provider integration required.</p></div><div class="card"><h3>🔥 LPG Services</h3><p>Provider integration required.</p></div></div></div>`}
function wallet(){$("#app").innerHTML=`<div class="container"><img src="jana-bazar-utilities.png" style="width:260px;max-width:100%" alt=""><div class="card"><h1>JANA Wallet</h1><h2>${money(db.wallet.balance)}</h2><p class="muted">Demo balance only. Real wallet money requires regulated/payment infrastructure and server-side ledger controls.</p></div></div>`}
